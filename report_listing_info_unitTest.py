...
use_coles =  ['amenities', 'price', 'city','description','accommodates', 'bathrooms','bedrooms','beds','room_type','state','name']
name_listings = 'C:\\Users\\Admin\\Documents\\school t1\\Software Tech\\Project\\data\\listings_dec18.csv'
name = 'C:\\Users\\Admin\\Documents\\school t1\\Software Tech\\Project\\data\\calendar_dec18.csv'
...
def listings_in_suburb(suburb,s,e, chunk): #return listings in a given suburb, within a user-selected period
    ...
    type_return = type(l) == list
    if type_return == False: #if the 
        return False
    for i in range(l):
        if isinstance(l[i], pd.Series) == False:
            return False
...
def keyword(keyword,s,e, chunk): #return listing based on a keyword(s) and user selected period
    ...
    type_return = type(amenities) == list
    if type_return == False:
        return False
    for i in range(amenities):
        if isinstance(amenities[i], pd.Series) == False:
            return False
...
def price_distribtion(suburb, s,e,name):     #price distribution
    noDate = False                       
    if len(s) == 0 or len(e) ==  0:
        noDate = True
    import matplotlib.pyplot as plt # plotting
    ...
    for x in range(len(listings)):
        ...
        if (str(listings.at[x,'city']).upper() == suburb or t) and compare:
    ...
    plt.ylabel('Total')
    if noDate:
        title = f'Price Distribution for all of {suburb}' #if no date was given
    else:
        title = f'Price Distribution for {suburb}, {s} to {e}' #if dates were given, show dates in title 
    plt.title(title)
